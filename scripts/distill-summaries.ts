...
import { redistillAllSummaries } from "../src/lib/generate-sector-analysis";
...
async function main() {
  ...
  console.log(`Re-distilling user summaries for ${label}...\n`);

  const results = await redistillAllSummaries(sector);
  for (const r of results) {
    console.log(`  ${r.success ? "✓" : "✗"} ${r.sector}${r.error ? `: ${r.error}` : ""}`);
  }

  const ok = results.filter((r) => r.success).length;
  console.log(`\nDone! ${ok}/${results.length} summaries distilled.`);
}
